# Restore uniy sample database

In this lesson, we'll restore the `uniy` sample database using the `uniy_db_backup.sql` script generated in the previous lesson.
...
```console
uniy=> \ir ./pgbackup/uniy_db_backup.sql
BEGIN
ALTER TABLE
ALTER TABLE
ALTER TABLE
...
4. **Describe tables and list records**
