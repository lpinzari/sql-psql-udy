# uniy schema and data backup

In the previous lesson we learned how to backup a database using the `pg_dump` command and save it to a **sql** script. However, the script includes `cleaning`, `creating` and `populating` the database tables. Sometimes, we only need to drop and create the database schema or just adding new records to an existing schema.
...
```console
(base) ludo /pgbackup  $  pg_dump -U usertest -F p -O --no-tablespaces -c --if-exists -s uniy > /Users/ludovicopinzari/pgbackup/uniy_db_schema_backup.sql
```

Let's execute the same editing of the previous lesson.

2. **remove settings database**:
...
We remove these lines. from line `8` to line `17`.

```console
(base) ludo /pgbackup  $  sed '8,17d' uniy_db_backup.sql > tmpfile && mv tmpfile uniy_db_schema_backup.sql
```

3. **transaction**

```console
(base) ludo /pgbackup  $  sed 's/-- Dumped from database version 11.4/BEGIN;/g' uniy_db_schema_backup.sql > tmpfile.sql && mv tmpfile.sql uniy_db_schema_backup.sql
```

`COMMIT`:

```console
(base) ludo /pgbackup  $  sed 's/-- PostgreSQL database dump complete/COMMIT;/g' uniy_db_schema_backup.sql > tmpfile.sql && mv tmpfile.sql uniy_db_schema_backup.sql
```

4. **clean**

```console
(base) ludo /pgbackup  $  sed 's/ALTER TABLE IF EXISTS ONLY public./ALTER TABLE IF EXISTS /g' uniy_db_schema_backup.sql > tmpfile.sql && mv tmpfile.sql uniy_db_schema_backup.sql
```

For dropping the tables:

```console
(base) ludo /pgbackup  $  sed 's/DROP TABLE IF EXISTS public./DROP TABLE IF EXISTS /g' uniy_db_schema_backup.sql > tmpfile.sql && mv tmpfile.sql uniy_db_schema_backup.sql
```

5. **create**

```console
(base) ludo /pgbackup  $  sed 's/CREATE TABLE public./CREATE TABLE /g' uniy_db_schema_backup.sql > tmpfile.sql && mv tmpfile.sql uniy_db_schema_backup.sql
```

6. **constraint** primary key and foreign key part:

```console
(base) ludo /pgbackup  $  sed 's/ALTER TABLE ONLY public./ALTER TABLE /g' uniy_db_schema_backup.sql > tmpfile.sql && mv tmpfile.sql uniy_db_schema_backup.sql
```
foreign key part

```console
(base) ludo /pgbackup  $  sed 's/REFERENCES public./REFERENCES /g' uniy_db_schema_backup.sql > tmpfile.sql && mv tmpfile.sql uniy_db_schema_backup.sql
```

## uniy_db_schema_backup.sql
...
```console
(base) ludo /pgbackup  $  pg_dump -U usertest -F p -O -a --column-inserts uniy > /Users/ludovicopinzari/pgbackup/uniy_db_data_backup.sql
```
...
```console
(base) ludo /pgbackup  $  sed '8,17d' uniy_db_data_backup.sql > tmpfile && mv tmpfile uniy_db_data_backup.sql
```

3. **transaction**

```console
(base) ludo /pgbackup  $  sed 's/-- Dumped from database version 11.4/BEGIN;/g' uniy_db_data_backup.sql > tmpfile.sql && mv tmpfile.sql uniy_db_data_backup.sql
```

`COMMIT`:

```console
(base) ludo /pgbackup  $  sed 's/-- PostgreSQL database dump complete/COMMIT;/g' uniy_db_data_backup.sql > tmpfile.sql && mv tmpfile.sql uniy_db_data_backup.sql
```

4. **data**

```console
(base) ludo /pgbackup  $  sed 's/INSERT INTO public./INSERT INTO /g' uniy_db_data_backup.sql > tmpfile.sql && mv tmpfile.sql uniy_db_data_backup.sql
```

## uniy_db_data_backup.sql
...
You may notice that the table order for the `INSERT INTO` statements are:
...
The order depends on the foreign key constraints.

In the next lesson, we'll execute the `uniy_db_schema_backup` and `uniy_db_data_backup` to restore the database state.
