# pg_dump: uniy Sql script Backup

In this lesson we show again all the steps done in the previous lesson to backup the `uniy` sample database. There is only a small difference in the execution order of the steps.

1. **pg_dump** backup:

```console
(base) ludo /pgbackup  $  pg_dump -U usertest -F p -O --no-tablespaces -c --if-exists --column-inserts uniy > /Users/ludovicopinzari/Documents/pgbackup/uniy_db_backup.sql
```
...
7. **constraint** primary key and foreign key part:
...
In the next lesson we'll execute the uniy_db_backup.sql file to restore the database.
